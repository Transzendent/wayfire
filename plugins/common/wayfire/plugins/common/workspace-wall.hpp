...
class workspace_wall_t
{
  public:
    ...
    void render_wall(const wf::framebuffer_t& fb, wf::geometry_t geometry)
    {
        ...
        /* Needs to be centered around (0, 0) */
  ...
  protected:
    ...
    glm::mat4 calculate_workspace_matrix(const wf::point_t& ws) const
    {
        ...
        auto inverse_transform = glm::inverse(
            output->render->get_target_framebuffer().transform);
        auto scaling = glm::scale(glm::mat4(1.0),
            glm::vec3{target_geometry.width / 2.0,
                target_geometry.height / 2.0, 1.0});
        auto translation = glm::translate(glm::mat4(1.0),
            glm::vec3{target_geometry.x + target_geometry.width / 2.0,
                target_geometry.y + target_geometry.height / 2.0, 0.0});
        return translation * scaling * inverse_transform;
    }
